...
data {
  ...
  int<lower=1> N2;
  array[N2] real x2;
  real ra; // rho prior (rate)
  real rb; // rho prior
  real alphal; // alpha prior (max cov)
  real alphah; // alpha prior
}
transformed data {
...
parameters {
  ...
  real<lower=alphal,upper=alphah> alpha;
  real<lower=0> sigma;
}
model {
  matrix[N1, N1] L_K;
  ...
  rho ~ beta(ra,rb);
  alpha ~ uniform(alphal,alphah);
  sigma ~ normal(0.25,0.25);
